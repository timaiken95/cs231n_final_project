...
for it in range(NUM_EPOCHS):
  ...
  for train_f in train_files:
    ...
    with tf.Session(graph=graph) as sess:
      ...
      while counter < numFiles:
        ...
        _, curr_rms, curr_loss = sess.run([train_step, rms, loss], feed_dict = {x: xtrain, y_: ytrain, keep_prob: DROPOUT})
  ...
  print("Training Loss for epoch " + str(it) + ":", total_train_loss)
  print("Training RMS for epoch " + str(it) + ":", total_train_rms)

  train_rmses.append(total_train_rms)
  ...
  for val_f in val_files:
    ...
    with tf.Session(graph=graph) as sess:
      ...
      while counter < numFiles:
        ...
        xtrain, ytrain = sess.run([images, labels])
        curr_rms, curr_loss = sess.run([rms, loss], 
                        feed_dict = {x: xtrain,
                               y_: ytrain, 
                               keep_prob: 1.0})
  ...
  print("Validation RMS for epoch " + str(it) + ":", total_val_rms)
  print("Validation Loss for epoch " + str(it) + ":", total_val_loss, "\n")

  val_rmses.append(total_val_rms)
